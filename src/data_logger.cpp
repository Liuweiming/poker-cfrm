...
void DataLoggerJsonLines::Write(DataLogger::Record record) {
  ...
  record.insert({
      {"time_str",
       json::Value(absl::FormatTime("%Y-%m-%d %H:%M:%E3S %z", now, utc))},
      {"time_abs", json::Value(absl::ToUnixMicros(now) / 1000000.)},
      {"time_rel", json::Value(absl::ToDoubleSeconds(now - start_time_))},
  });
